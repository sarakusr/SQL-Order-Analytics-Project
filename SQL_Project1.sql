...
---Top 3 outlet by cusine type without using limit and top function
with cte as(select Cuisine,restaurant_id,count(*) as_number_of_orders
from orders
group by Cuisine,restaurant_id)
select * from (select *
,ROW_NUMBER()over(partition by Cuisine order by as_number_of_orders desc) as rn
from cte) a
where rn<=2

---Find the daily new customer from the launch date( everyday day how many new customer we are acquiring)
with cte as(select Customer_code,cast(min(placed_at) as date) as first_order
from orders
group by Customer_code)
select first_order,count(*) as no_of_new_customer
from cte
group by first_order
order by first_order
--- Count of all the users who were acquired in jan 2025 and only placed one order in jan and did not place anyother order

select customer_code,count(*) as no_of_orders
from orders
where month(placed_at)=1 AND year(placed_at)=2025
and Customer_code not in( select distinct Customer_code from
orders
where not (month(placed_at)=1 AND year(placed_at)=2025))
group by Customer_code
having count(*)=1

--- List all the customers with no orders in last 7 days but were acquired one month ago with there first order on promo
...
---list customers who placed more then 1 order and all there orders are on promo only 
...
--- What % of customers were organically Acquired in jan 2025, (Placed there first order without promocode)
with cte as(select * 
,row_number()over(partition by customer_code order by placed_at) as rn
from orders
where month(placed_at)=1) 
select count(case when rn=1 and promo_code_name is null then customer_code end)*100.0/count(distinct customer_code) as percentage_ofcustomer
from cte 
